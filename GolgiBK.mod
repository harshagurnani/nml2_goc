TITLE Mod file for component: Component(id=GolgiBK type=ionChannelHH)
...
PARAMETER {
    ...
    c_instances = 1 
    c_forwardRate_rate = 7.0000005 (kHz)
    c_forwardRate_caScale = 0.0015 (mM)
    c_forwardRate_scale = -11.765 (mV)
    c_reverseRate_rate = 1 (kHz)
    c_reverseRate_caScale = 1.5E-4 (mM)
    c_reverseRate_scale = -11.765 (mV)
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=GolgiBK type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=GolgiBK type=ionChannelHH), from gates; Component(id=c type=gateHHrates)
...
PROCEDURE rates() {
    ...
    c_forwardRate_r = c_forwardRate_rate *1/( 1 +  c_forwardRate_caScale /caConc * exp(v/ c_forwardRate_scale )) ? evaluable
    c_reverseRate_r = c_reverseRate_rate *1/( 1 + caConc/ c_reverseRate_caScale  * 1/exp(v/ c_reverseRate_scale )) ? evaluable
    ...
    ? DerivedVariable is based on path: forwardRate/r, on: Component(id=c type=gateHHrates), from forwardRate; Component(id=null type=Golgi_BK_alpha)
    c_alpha = c_forwardRate_r ? path based, prefix = c_
    
    ? DerivedVariable is based on path: reverseRate/r, on: Component(id=c type=gateHHrates), from reverseRate; Component(id=null type=Golgi_BK_beta)
