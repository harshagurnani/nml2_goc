TITLE Mod file for component: Component(id=GolgiKM type=ionChannelHH)
...
PARAMETER {
    ...
    n_steadyState_midpoint = -35 (mV)
    n_steadyState_scale = 6 (mV)
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=GolgiKM type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=GolgiKM type=ionChannelHH), from gates; Component(id=n type=gateHHratesInf)
...
PROCEDURE rates() {
    ...
    ? DerivedVariable is based on path: steadyState/x, on: Component(id=n type=gateHHratesInf), from steadyState; Component(id=null type=HHSigmoidVariable)
