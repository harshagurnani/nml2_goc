TITLE Mod file for component: Component(id=GolgiCaLVA type=ionChannelHH)
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=GolgiCaLVA type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=GolgiCaLVA type=ionChannelHH), from gates; Component(id=m type=gateHHtauInf)
...
PROCEDURE rates() {
    ...
    m_timeCourse_t = m_timeCourse_TIME_SCALE  * (3 + 1/(exp(( m_timeCourse_V +27)/10) + exp(-( m_timeCourse_V +102)/15) )) ? evaluable
    ...
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=m type=gateHHtauInf), from timeCourse; Component(id=null type=Golgi_CaLVA_taum)
    ...
    h_timeCourse_t = h_timeCourse_TIME_SCALE  * (85 + 1/(exp(( h_timeCourse_V +48)/4) + exp(-( h_timeCourse_V +407)/50) )) ? evaluable
    ...
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=h type=gateHHtauInf), from timeCourse; Component(id=null type=Golgi_CaLVA_tauh)
