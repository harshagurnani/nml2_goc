TITLE Mod file for component: Component(id=GolgiGIRK type=ionChannelHH)
...
PARAMETER {
    ...
    m_steadyState_midpoint = -82 (mV)
    m_steadyState_scale = -13 (mV)
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=GolgiGIRK type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=GolgiGIRK type=ionChannelHH), from gates; Component(id=m type=gateHHtauInf)
...
PROCEDURE rates() {
    ...
    m_timeCourse_t = m_timeCourse_TIME_SCALE  * (1.96178 + 1.82208*exp(-( m_timeCourse_V +41.7348)*( m_timeCourse_V +41.7348)/706.8499)) ? evaluable
    ...
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=m type=gateHHtauInf), from timeCourse; Component(id=null type=Golgi_GIRK_taum)
