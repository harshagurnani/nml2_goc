TITLE Mod file for component: Component(id=GolgiKA type=ionChannelHH)
...
NEURON {
    ...
    RANGE a_reverseRate_rate                : parameter
    RANGE a_reverseRate_midpoint            : parameter
    RANGE a_reverseRate_scale               : parameter
...
PARAMETER {
    ...
    a_reverseRate_rate = 0.16550002 (kHz)
    a_reverseRate_midpoint = -18.27914 (mV)
    a_reverseRate_scale = 19.47175 (mV)
    ...
    b_reverseRate_midpoint = -49.9537 (mV)
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=GolgiKA type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=GolgiKA type=ionChannelHH), from gates; Component(id=a type=gateHHratesInf)
...
PROCEDURE rates() {
    ...
    a_reverseRate_r = a_reverseRate_rate *1/exp((v- a_reverseRate_midpoint )/ a_reverseRate_scale ) ? evaluable
    ...
    ? DerivedVariable is based on path: reverseRate/r, on: Component(id=a type=gateHHratesInf), from reverseRate; Component(id=null type=Golgi_KA_abeta)
