TITLE Mod file for component: Component(id=GolgiNaP type=ionChannelHH)
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=GolgiNaP type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=GolgiNaP type=ionChannelHH), from gates; Component(id=m type=gateHHratesInf)
...
PROCEDURE rates() {
    ...
    ? DerivedVariable is based on path: steadyState/x, on: Component(id=m type=gateHHratesInf), from steadyState; Component(id=null type=HHSigmoidVariable)
