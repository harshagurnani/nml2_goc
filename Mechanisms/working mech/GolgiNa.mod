TITLE Mod file for component: Component(id=GolgiNa type=ionChannelHH)
...
PARAMETER {
    ...
    h_reverseRate_rate = 3.0000002 (kHz)
    h_reverseRate_midpoint = -17 (mV)
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=GolgiNa type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=GolgiNa type=ionChannelHH), from gates; Component(id=m type=gateHHratesTau)
    ? multiply applied to all instances of fcond in: <gates> ([Component(id=m type=gateHHratesTau), Component(id=h type=gateHHrates)]))
...
PROCEDURE rates() {
    ...
    if (( m_timeCourse_ALPHA  +  m_timeCourse_BETA ) == 0)  { 
        m_timeCourse_t = 1000.0  *  m_timeCourse_TIME_SCALE ? evaluable cdv
    } else if (1/( m_timeCourse_ALPHA  +  m_timeCourse_BETA )  < ( 0.05 ))  { 
        m_timeCourse_t = 0.05  *  m_timeCourse_TIME_SCALE ? evaluable cdv
    } else  { 
    ...
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=m type=gateHHratesTau), from timeCourse; Component(id=null type=Golgi_NaT_m_tau)
