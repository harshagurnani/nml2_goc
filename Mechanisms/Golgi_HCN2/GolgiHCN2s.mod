TITLE Mod file for component: Component(id=GolgiHCN2s type=ionChannelHH)
...
NEURON {
    ...
    RANGE s_steadyState_rvi                : conditional derived var
...
ASSIGNED {
    ...
    s_steadyState_rvi                      : conditional derived var...
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=GolgiHCN2s type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=GolgiHCN2s type=ionChannelHH), from gates; Component(id=s type=gateHHtauInf)
...
PROCEDURE rates() {
    ...
    s_timeCourse_t2 = exp((0.0152 *  s_timeCourse_V ) + 5.2944)* 2.3026 ? evaluable
    if (s_timeCourse_V  > -20)  { 
        s_timeCourse_t = 500 *  s_timeCourse_TIME_SCALE ? evaluable cdv
    } else  { 
    ...
    if (s_steadyState_V   < ( -64.70  ))  { 
        s_steadyState_rvi = 1- ((-0.0227  *  s_steadyState_V )  -1.4694) ? evaluable cdv
    } else  { 
        s_steadyState_rvi = 1 ? evaluable cdv
    }
    
    if (s_steadyState_rvi   > 1)  { 
        s_steadyState_x = 1 * (1 / (1 + (exp (( s_steadyState_V  + 81.95) * 0.1661) ) )) ? evaluable cdv
    } else if (s_steadyState_rvi   < 0)  { 
        s_steadyState_x = 0.001 ? evaluable cdv
    } else  { 
        s_steadyState_x = s_steadyState_rvi  * (1 / (1 + (exp (( s_steadyState_V  + 81.95) * 0.1661) ) )) ? evaluable cdv
    }
    ...
    ? DerivedVariable is based on path: steadyState/x, on: Component(id=s type=gateHHtauInf), from steadyState; Component(id=null type=Golgi_HCN2s_oinf)
    s_inf = s_steadyState_x ? path based, prefix = s_
    
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=s type=gateHHtauInf), from timeCourse; Component(id=null type=Golgi_HCN2s_tau)
